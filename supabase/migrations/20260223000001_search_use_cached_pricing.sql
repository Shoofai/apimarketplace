-- Use cached min/max price on apis in search_marketplace_apis (no LATERAL/EXISTS for price).
...
BEGIN
  ...
  WITH base AS (
    SELECT
      ...
      a.original_url,
      a.total_api_calls,
      a.min_price_cached AS min_price,
      a.max_price_cached AS max_price,
      o.name AS org_name,
      o.slug AS org_slug,
    ...
    WHERE a.status IN ('published', 'unclaimed')
      ...
      AND (NOT p_free_only OR a.min_price_cached = 0)
      AND (p_tags IS NULL OR array_length(p_tags, 1) IS NULL OR a.tags @> p_tags)
      AND (p_price_min IS NULL OR a.max_price_cached >= p_price_min)
      AND (p_price_max IS NULL OR (a.min_price_cached IS NOT NULL AND a.min_price_cached <= p_price_max))
  ),
