-- RPC for marketplace search with price/free/tags filters and price sort.
-- Returns one row: (data jsonb, total bigint). data = array of api objects with org/category/pricing.
...
DECLARE
  ...
  v_pattern text;
BEGIN
  -- Escape % and _ for literal ILIKE (and backslash)
  ...
  WITH base AS (
    SELECT
      ...
      c.name AS cat_name,
      c.slug AS cat_slug,
      (SELECT MIN(pp.price_monthly) FROM api_pricing_plans pp WHERE pp.api_id = a.id) AS min_price,
      (SELECT MAX(pp.price_monthly) FROM api_pricing_plans pp WHERE pp.api_id = a.id) AS max_price
    FROM apis a
