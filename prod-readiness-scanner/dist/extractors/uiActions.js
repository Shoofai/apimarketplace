...
function isStubHandler(body) {
    const t = body.trim();
    if (!t || t === '{}' || t === ';')
        return true;
    if (/^console\.\s*(log|warn|error|info)\s*\(/.test(t))
        return true;
    if (/^toast\s*\(/.test(t))
        return true;
    if (/^alert\s*\(/.test(t))
        return true;
    if (/return\s*;?\s*$/.test(t) && t.length < 20)
        return true;
    if (STUB_PATTERNS.test(t))
        return true;
    return false;
}
function getLabelFromJsx(node) {
...
export function extractUiActions(graph, options) {
        ...
        for (const el of jsx) {
            ...
            for (const attr of attrs) {
                ...
                if ((name === 'onClick' || name === 'onSubmit') && init) {
                    ...
                    if (idMatch)
                        handlerName = idMatch[1];
                    if (isStubHandler(text))
                        suspicious = true;
                }
            }
