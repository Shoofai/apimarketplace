...
export function buildFileIndex(options) {
    const { projectRoot, include = DEFAULT_INCLUDE, excludeDirs = DEFAULT_EXCLUDE_DIRS, excludePaths = [], } = options;
    const results = [];
    const seen = new Set();
    function walk(dir) {
        let entries;
        ...
        for (const ent of entries) {
            ...
            if (ent.isDirectory()) {
                if (excludeDirs.some((d) => rel.startsWith(d) || ent.name === d))
                    continue;
                if (excludePaths.some((p) => full.includes(p) || rel.startsWith(p)))
                    continue;
                walk(full);
                continue;
            }
