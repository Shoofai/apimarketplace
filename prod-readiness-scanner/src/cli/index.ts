...
if (command === 'scan') {
  let project = '.';
  let out = './audit';
  let format: ('json' | 'md')[] = ['json', 'md'];
  let failOn: 'CRITICAL' | 'HIGH' | 'MEDIUM' | 'LOW' = 'CRITICAL';
  ...
  for (let i = 1; i < args.length; i++) {
    if (args[i] === '--project' && args[i + 1]) {
      ...
      format = f.filter((s) => s === 'json' || s === 'md') as ('json' | 'md')[];
      if (format.length === 0) format = ['json', 'md'];
    } else if (args[i] === '--fail-on' && args[i + 1]) {
      const v = args[++i].toUpperCase();
      if (['CRITICAL', 'HIGH', 'MEDIUM', 'LOW'].includes(v)) {
        failOn = v as 'CRITICAL' | 'HIGH' | 'MEDIUM' | 'LOW';
      }
    }
  }
