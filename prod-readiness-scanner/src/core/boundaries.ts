...
export function getBoundaryInfo(filePath: string): BoundaryInfo {
  let content: string;
  try {
  ...
  for (const line of lines) {
    ...
    if (trimmed.startsWith('"') || trimmed.startsWith("'")) {
      if (trimmed.includes(DIRECTIVE_CLIENT)) isClient = true;
      if (trimmed.includes(DIRECTIVE_SERVER)) isServerAction = true;
    }
