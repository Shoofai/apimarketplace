...
export function extractMigrations(graph: AppGraph, options: MigrationsExtractorOptions): void {
  const { files } = options;
  ...
  for (const f of migrationFiles) {
    ...
    const tablesRls = new Map<string, boolean>();
    const tablesPolicies = new Map<string, number>();
    const tablesIndex = new Set<string>();
    ...
    while ((m = RLS_ENABLE.exec(content)) !== null) {
      ...
      if (tableMatch) tablesRls.set(tableMatch[1], true);
    }
    ...
    while ((m = CREATE_POLICY.exec(content)) !== null) {
      ...
      if (tableName) {
        ...
        if (normalized !== tableName) tablesPolicies.set(tableName, (tablesPolicies.get(tableName) ?? 0) + 1);
      }
    }
    CREATE_INDEX.lastIndex = 0;
    while ((m = CREATE_INDEX.exec(content)) !== null) {
      const tableMatch = content.slice(m.index).match(/ON\s+([\w."]+)/i);
      if (tableMatch) tablesIndex.add(tableMatch[1]);
    }
    if (DESTRUCTIVE.test(content)) hasDestructive = true;
