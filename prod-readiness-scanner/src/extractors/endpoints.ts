...
export function extractEndpoints(graph: AppGraph, options: EndpointsExtractorOptions): void {
  ...
  for (const f of files) {
    ...
    if ((rel.includes('/route.') && rel.startsWith('src/app')) || (rel.includes('/route.') && rel.startsWith('app'))) {
      ...
      for (const method of HTTP_METHODS) {
        ...
        const first = decls[0];
        const line = 'getStartLineNumber' in first ? (first as { getStartLineNumber(): number }).getStartLineNumber() : undefined;
