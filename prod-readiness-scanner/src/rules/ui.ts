...
function routePathToUrlForm(routePath: string): string {
  const withoutGroups = routePath.replace(/\/\([^)]+\)/g, '').replace(/\/+/g, '/');
  return withoutGroups || '/';
}

export function runUiRules(graph: AppGraph, context: RuleContext): Finding[] {
    ...
    if (action.suspicious) {
      findings.push(
        createFinding(
          ...
          'MEDIUM',
          'HIGH',
          'Suspicious or unwired UI action',
          `Button/link may be unwired or stub: ${action.label || action.element} (${action.filePath})`,
          action.filePath,
          action.line,
          'Wire to a real handler or remove placeholder.'
        )
      );
    }
    if (action.href) {
      ...
      findings.push(
        createFinding(
          ...
          'HIGH',
          'Dead link',
          `Link href "${action.href}" does not match any discovered route.`,
          action.filePath,
          action.line,
          'Fix href to a valid route or use Link with existing path.'
        )
      );
      }
    }
    ...
    if (!calledPaths.has(route.path)) {
      findings.push(
        createFinding(
          ...
          'MEDIUM',
          'Orphan API route',
          `API route ${route.path} may never be called (no matching callsite).`,
          route.filePath,
          undefined,
          'Confirm route is used (e.g. from server action or external).'
        )
      );
    }
  }
