...
export function runSecurityRules(graph: AppGraph, context: RuleContext): Finding[] {
    ...
    if (ev.isPublic && (ev.name.includes('SERVICE') || ev.name.includes('service_role'))) {
      findings.push(
        createFinding(
          ...
          'CRITICAL',
          'HIGH',
          'Service role or secret in public env',
          `NEXT_PUBLIC_ or public env may expose service role: ${ev.name}`,
          ev.filePath,
          ev.line,
          'Never use NEXT_PUBLIC_ for service role or secrets.'
        )
      );
    }
    if (NEXT_PUBLIC_SECRET.test(ev.name) && !ALLOWED_PUBLIC_KEYS.has(ev.name)) {
      findings.push(
        createFinding(
          ...
          'HIGH',
          'HIGH',
          'NEXT_PUBLIC_ may expose secret',
          `Env var name suggests secret/token: ${ev.name}`,
          ev.filePath,
          ev.line,
          'Rename or use server-only env.'
        )
      );
    }
  }
