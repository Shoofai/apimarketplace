...
export function runDbRules(graph: AppGraph, _context: RuleContext): Finding[] {
    ...
    if (m.table && m.rlsEnabled === false) {
      findings.push(
        createFinding(
          ...
          'HIGH',
          'Table without RLS',
          `Table ${m.table} may not have RLS enabled.`,
          m.filePath,
          undefined,
    ...
    if (m.table && m.rlsEnabled === true && (m.policyCount ?? 0) === 0) {
      findings.push(
        createFinding(
          ...
          `Table ${m.table} has RLS but no CREATE POLICY found.`,
          m.filePath,
          undefined,
          'Add at least one CREATE POLICY for the table.'
        )
      );
    }
    if (m.hasDestructiveDdl) {
      findings.push(
        createFinding(
          ...
          'Migration contains DROP TABLE/COLUMN, TRUNCATE, or ALTER COLUMN TYPE.',
          m.filePath,
          undefined,
          'Ensure backup/rollback plan; avoid destructive DDL in shared branches.'
        )
      );
    }
  }
