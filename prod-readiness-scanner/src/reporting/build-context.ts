...
import type { Finding, ValidationContext, Gap, RouteEntry, ShipChecklistItem, ShipStatus } from '../core/types.js';
...
function findingToGap(f: Finding, suppressed: boolean): Gap {
  return {
    id: f.id,
...
export function isFindingSuppressed(finding: Finding, baseline: ValidationBaseline | null): boolean {
  if (!baseline?.suppress?.length) return false;
  for (const s of baseline.suppress) {
    if (s.ruleId !== finding.code) continue;
    if (!s.gapId && !s.filePath) return true;
    if (s.gapId && s.gapId === finding.id) return true;
    if (s.filePath && finding.evidence[0]?.filePath?.includes(s.filePath)) {
      if (s.line == null || finding.evidence[0]?.line === s.line) return true;
    }
  }
  return false;
}

function isSuppressed(finding: Finding, baseline: ValidationBaseline | null): boolean {
...
export function buildValidationContext(
  ...
  for (const f of findings) {
    ...
    if (suppressed) suppressedIds.add(f.id);
    gaps.push(findingToGap(f, suppressed));
  }

  const routes: RouteEntry[] = getNodesByKind(graph, 'Route').map((r) => ({
    path: r.path,
    status: 'ok',
    description: r.isApi ? `API: ${r.path}` : `Page: ${r.path}`,
  }));

  const shipChecklist: ShipChecklistItem[] = [
  ...
  const hasHighAuthSec = activeFindings.some(
    (f) => f.severity === 'HIGH' && (f.category === 'auth' || f.category === 'security')
  );
  let shipChecklistStatus: ShipStatus = 'ship';
  if (hasCritical) shipChecklistStatus = 'no-ship';
  else if (hasHighAuthSec) shipChecklistStatus = 'needs-review';
